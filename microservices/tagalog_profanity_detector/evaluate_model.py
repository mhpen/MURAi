...
MODEL_VERSION = os.environ.get('MODEL_VERSION', 'v1.3.0')

# Model path - use the same model as in app.py
MODEL_PATH = os.environ.get('MODEL_PATH', 'jcblaise/roberta-tagalog-large')
...
def main():
    ...
    try:
        ...
        if success:
            save_model_log("info", f"Model {MODEL_VERSION} evaluated successfully with accuracy {metrics['accuracy']:.4f}")
            logger.info("Evaluation completed and metrics saved successfully")
        else:
            save_model_log("error", f"Failed to save evaluation metrics for model {MODEL_VERSION}")
            logger.error("Failed to save metrics to database")

        return metrics
    except Exception as e:
        logger.error(f"Error in evaluation process: {str(e)}")
        save_model_log("error", f"Error evaluating model {MODEL_VERSION}: {str(e)}")
        return None
