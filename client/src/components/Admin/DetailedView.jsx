...
import ModelTestMetricsAnalytics from './ModelTestMetricsAnalytics';
...
const DetailedView = ({ isDarkMode }) => {
      ...
      <div className="flex items-center gap-2">
        <button
          className={cn(
            ...
            isDarkMode
              ? "border-gray-800 bg-transparent text-gray-400 hover:bg-gray-700 hover:text-white"
              : "border-gray-200 bg-white text-black/70 hover:bg-black/5 hover:text-black"
          )}
          onClick={refreshData}
  ...
  // References for model test metrics charts
  const responseTimeChartRef = useRef(null);
  const confidenceChartRef = useRef(null);
  const textLengthChartRef = useRef(null);
  const modelComparisonChartRef = useRef(null);
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      const charts = {
        ...
        // Model test metrics charts
        responseTime: responseTimeChartRef.current?.canvas?.toDataURL(),
        confidence: confidenceChartRef.current?.canvas?.toDataURL(),
        textLength: textLengthChartRef.current?.canvas?.toDataURL(),
        modelComparison: modelComparisonChartRef.current?.canvas?.toDataURL()
      };
  ...
  return (
    <div className="space-y-8">
      ...
      {/* Model Test Metrics Analytics */}
      <div className="mt-12">
        <ModelTestMetricsAnalytics
          isDarkMode={isDarkMode}
          responseTimeChartRef={responseTimeChartRef}
          confidenceChartRef={confidenceChartRef}
          textLengthChartRef={textLengthChartRef}
          modelComparisonChartRef={modelComparisonChartRef}
        />
      </div>
    </div>
  );
